...
char ball_paddle_collision()
{
  ...
  if ((ball_position[1]  >= paddle_Col_Coordintates[0] - 1) && (ball_position[1] <= paddle_Col_Coordintates[1] - 1))
    if ( (ball_position[0] + 6) >=  paddle_position[0])   // on same row coordinates
      return 1;
  return 0;
}
